...
output "master_private_ip" {
  description = "Private IP address of the K8s master node (no public IP)."
  value       = google_compute_instance.k8s_master.network_interface[0].network_ip
}

output "worker_private_ips" {
  description = "Private IP addresses of all K8s worker nodes (no public IPs)."
  value       = [for worker in google_compute_instance.k8s_worker : worker.network_interface[0].network_ip]
}

output "ingress_public_ip" {
  description = "Public VIP of the external Network Load Balancer (HTTP/HTTPS front door)."
  value       = try(google_compute_address.ingress_lb[0].address, null)
}
